...
- (void)layoutIndicatorViews {
    ...
    if (_verticalAlign) {
        [self layoutVerticalIndicatorViews];
    }else {
        [self layoutHorizenlIndicatorViews];
    }
}

- (void)layoutHorizenlIndicatorViews {
    CGFloat orignX = 0;
    CGFloat centerY = 0;
...
- (void)layoutVerticalIndicatorViews {
    CGFloat orignY = 0;
    CGFloat centerX = CGRectGetWidth(self.frame)/2;;
    CGFloat pageIndicatorSpaing = _pageIndicatorSpaing;
    orignY = (CGRectGetHeight(self.frame) - (_indicatorViews.count - 1) * (_pageIndicatorSize.height + pageIndicatorSpaing) - _currentPageIndicatorSize.height)/2;
    NSInteger index = 0;
    for (UIImageView *indicatorView in _indicatorViews) {
        if (_pageIndicatorImage) {
            indicatorView.layer.cornerRadius = 0;
        ...
        CGSize size = index == _currentPage ? _currentPageIndicatorSize : _pageIndicatorSize;
        indicatorView.frame = CGRectMake(centerX - size.width/2 , orignY, size.width, size.height);
        orignY += size.height + pageIndicatorSpaing;
        ++index;
    }
}

...
@interface TYCyclePagerView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, TYCyclePagerTransformLayoutDelegate> {
    struct {
        ...
        unsigned int didScrollFromIndexSectionToNewIndexSection  :1;
...
@property (nonatomic, assign) TYIndexSection scrollToIndexSection;
@property (nonatomic, assign) TYIndexSection scrollFromIndexSection;
...
- (void)setDelegate:(id<TYCyclePagerViewDelegate>)delegate {
    ...
    _delegateFlags.didScrollFromIndexSectionToNewIndexSection = [delegate respondsToSelector:@selector(pagerView:didScrollFromIndexSection:toIndexSection:)];
    ...
    _delegateFlags.willDisplayCell = [delegate respondsToSelector:@selector(pagerView:willDisplayCell:forIndex:)];
    _delegateFlags.didEndDisplayingCell = [delegate respondsToSelector:@selector(pagerView:didEndDisplayingCell:forIndex:)];
...
- (void)scrollToItemAtIndex:(NSInteger)index animate:(BOOL)animate {
    [self scrollToItemAtIndex:index onCurrentSection:NO animate:animate];
}

- (void)scrollToItemAtIndex:(NSInteger)index onCurrentSection:(BOOL)currentSection animate:(BOOL)animate {
    ...
    [self scrollToItemAtIndexSection:TYMakeIndexSection(index, index >= self.curIndex || currentSection ? _indexSection.section : _indexSection.section+1) animate:animate];
}
...
- (void)scrollToItemAtIndexSection:(TYIndexSection)indexSection animate:(BOOL)animate {
    ...
    _isScrollToIndex = [self isValidIndexSection:_indexSection];
    if (_isScrollToIndex && _ignoreScrollMiddleIndex) {
        _scrollFromIndexSection = _indexSection;
        _scrollToIndexSection = indexSection;
    }
...
- (nullable UICollectionViewCell *)cellForItemAtIndexSection:(TYIndexSection)indexSection {
    NSIndexPath *indexPath = [NSIndexPath indexPathForItem:indexSection.index inSection:indexSection.section];
...
- (void)setNeedUpdateLayout {
    ...
    [self clearLayout];
    [self updateLayout];
//    if (_layout.estimatedItemSize.width > 0) {
//        [_collectionView reloadData];
//    }else{
...
- (CGFloat)caculateOffsetXAtIndexSection:(TYIndexSection)indexSection{
    ...
    CGFloat itemWidth = layout.itemSize.width + layout.minimumInteritemSpacing;
    CGFloat offsetX = 0;
    if (_layout.itemAlign == TYCyclePagerLayoutAlignLeft) {
         if (_isInfiniteLoop || indexSection.index < _numberOfItems - 1) {
             ...
             offsetX = itemWidth*(indexSection.index + indexSection.section*_numberOfItems) - (width-layout.itemSize.width-leftEdge-rightEdge);
         }
    }else if (!_isInfiniteLoop && !_layout.itemHorizontalCenter && indexSection.index == _numberOfItems - 1) {
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if (_delegateFlags.willDisplayCell) {
        [_delegate pagerView:self willDisplayCell:cell forIndex:indexPath.row];
    }
}

- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if (_delegateFlags.didEndDisplayingCell) {
        [_delegate pagerView:self didEndDisplayingCell:cell forIndex:indexPath.row];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if ((_delegateFlags.didScrollFromIndexToNewIndex || _delegateFlags.didScrollFromIndexSectionToNewIndexSection) && !TYEqualIndexSection(_indexSection, indexSection)) {
        NSLog(@"进入");
        if (_ignoreScrollMiddleIndex && _isScrollToIndex && !TYEqualIndexSection(_scrollToIndexSection,_indexSection)) {
            // ignore
        }else if (_ignoreScrollMiddleIndex && _isScrollToIndex) {
            _isScrollToIndex = NO;
            if (_delegateFlags.didScrollFromIndexToNewIndex) {
                [_delegate pagerView:self didScrollFromIndex:MAX(_scrollFromIndexSection.index, 0) toIndex:_indexSection.index];
            }
            if (_delegateFlags.didScrollFromIndexSectionToNewIndexSection) {
                [_delegate pagerView:self didScrollFromIndexSection:_scrollFromIndexSection toIndexSection:_indexSection];
            }
        } else {
            _isScrollToIndex = NO;
            if (_delegateFlags.didScrollFromIndexToNewIndex) {
            ...
            if (_delegateFlags.didScrollFromIndexSectionToNewIndexSection) {
                [_delegate pagerView:self didScrollFromIndexSection:indexSection toIndexSection:_indexSection];
            }
        }
    }
}

...
- (void)setLayout:(TYCyclePagerViewLayout *)layout {
    ...
    self.estimatedItemSize = _layout.estimatedItemSize;
...
- (BOOL)itemHorizontalCenter {
    if (_itemVerticalCenter && _itemAlign == TYCyclePagerLayoutAlignCenter) {
        return YES;
    }
    return NO;
}
...
- (UIEdgeInsets)onlyOneSectionInset {
    CGFloat leftSpace = _sectionInset.left;
    if (_pageView && !_isInfiniteLoop && self.itemHorizontalCenter) {
        leftSpace = (CGRectGetWidth(_pageView.frame) - _itemSize.width)/2;
    }
    CGFloat rightSpace = _sectionInset.right;
    if (_pageView && !_isInfiniteLoop && self.itemHorizontalCenter) {
        rightSpace = (CGRectGetWidth(_pageView.frame) - _itemSize.width)/2;
    }else if (_itemAlign == TYCyclePagerLayoutAlignLeft) {
        rightSpace = _sectionInset.right;//CGRectGetWidth(_pageView.frame) - _itemSize.width-_sectionInset.left;
    }
